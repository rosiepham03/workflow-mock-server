...
public class WorkflowController {
    ...
    public ResponseEntity<?> getReturnOptions(
        ...
        boolean isCurrentApprover = "READY".equals(actionStep.getStatus()) || "IN_PROGRESS".equals(actionStep.getStatus());
        ...
        for (WorkflowStepDTO step : workflowDataService.getWorkflowData().getWorkflowSteps()) {
            ...
            for (StepApproverDTO sa : step.getStepApprovers()) {
                ...
                String approverLabel = (sa.getApprover() != null && sa.getApprover().getFullName() != null) 
                ...
                if (isCurrentApprover) {
                    ...
                    if (step.getSortOrder() < actionStep.getSortOrder() && "APPROVED".equals(saStatus)) {
                    ...
                    boolean isBefore = step.getSortOrder() < actionStep.getSortOrder() && "APPROVED".equals(saStatus);
                    boolean isCurrentActiveStep = "READY".equals(step.getStatus()) || "IN_PROGRESS".equals(step.getStatus());

                    if (isBefore || isCurrentActiveStep) {
                        ReturnOptionDTO option = new ReturnOptionDTO();
                        option.setCode(isCurrentActiveStep ? "RETURN_TO_CURRENT_APPROVER" : "RETURN_TO_PREVIOUS_APPROVER");
                        option.setTargetStepId(step.getId());
                        option.setApproverId(sa.getApproverId());
    ...
    public ResponseEntity<?> updateWorkflowStepStatus(
        ...
        switch (outcome) {
            ...
            case "REASSIGN":
                ...
                if (request.getTargetId() != null) {
                    ...
                    newApprover.setFullName(request.getActionBy() != null ? request.getActionBy() : "Người Nhận Ủy Quyền Mới");
            ...
            default:
                return ResponseEntity.badRequest().body(
                        Map.of("error", "Action '" + request.getStatusCode() + "' không hợp lệ. Cho phép: APPROVE, REJECT, RETURN, REASSIGN, ON_HOLD")
                );
        }
        ...
        if (prId != null && allPRs != null && allPRs.containsKey(prId)) {
            ...
            List<Map<String, Object>> historyLog = (List<Map<String, Object>>) paymentRequest.computeIfAbsent("history_log", k -> new ArrayList<>());
    ...
    public ResponseEntity<?> updatePRStatus(@RequestBody Map<String, Object> requestBody) {
        ...
        Map<String, Object> logEntry = new HashMap<>();
        logEntry.put("step", paymentRequest.getOrDefault("current_step", "DIRECT_UPDATE"));
        logEntry.put("approver", approverId != null ? approverId : "UNKNOWN_APPROVER");
    ...
    public ResponseEntity<?> getValidReassignees(@RequestBody Map<String, String> requestBody) {
        ...
        log.info("[POST] /api/workflow/get-valid-reassignees -> Tìm đồng cấp cho: {}", currentApproverId);
        ...
        // Lọc theo điều kiện Node.js: Active && Cùng Role && Không trùng ID bản thân
